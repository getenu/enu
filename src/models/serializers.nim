...
proc save_ide_support(level_dir: string, sorted_scripts: seq[string]) =
  let imports = sorted_scripts.map_it("import generated/" & it).join("\n")
  try_write_autogenerated(
    level_dir / "project.nim", autogenerated_header & imports & "\n"
  )
  try_write_autogenerated(
    level_dir / "project.nimble", autogenerated_header & "srcDir = \".\"\n"
  )
  let nim_cfg =
    autogenerated_header & "--path:\"" & state.config.lib_dir / "enu" &
    "\"\n--path:\"" & state.config.lib_dir / "stdlib" & "\"\n"
  try_write_autogenerated(level_dir / "nim.cfg", nim_cfg)
  try_write_autogenerated(
    level_dir / ".gitignore",
