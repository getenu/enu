...
proc claude_settings_content(): string =
  # Suggests the Enu plugin (skills + commands) via the marketplace published
  # from a path in the open-source repo. On folder-trust Claude prompts to
  # install it; nothing is written to the user's global config by Enu.
  let node =
    %*{
      "//": autogenerated_json_notice,
      "extraKnownMarketplaces":
        {"getenu": {"source": {"source": "github", "repo": "getenu/enu"}}},
      "enabledPlugins": {"enu@getenu": true},
    }
  node.pretty & "\n"
...
proc save_agent_support(level_dir: string) =
  ...
  # .claude/settings.json — marketplace + plugin auto-suggest.
  create_dir(level_dir / ".claude")
  try_write_autogenerated(
    level_dir / ".claude" / "settings.json", claude_settings_content()
  )
