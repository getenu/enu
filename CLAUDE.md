...
The build system supports building for different architectures. Pass `amd64` or `arm64` to `nim prereqs` to set the target architecture. The choice is persisted in `.build_arch` and automatically used by subsequent `nim build` commands.
...
**Required packages for amd64 builds on ARM64 host:**
```bash
# Cross-compiler
...
**Required packages for arm64 native builds:**
```bash
...
**Rosetta setup for running x86_64 Nim on ARM64 Linux (Parallels VM):**
