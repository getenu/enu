...
6. Add the `<id>` to `level.json`'s load order array
7. Touch all modified files, wait 5 seconds, `screenshot` to verify
8. Iterate until it looks right

Positioning: set the JSON `origin` to place the build in world space. Script coordinates are local (relative to origin).

If the structure is purely static voxels with no animation or parameters, you can skip the script and add all blocks directly in the JSON `edits` array — no `level.json` entry needed.
