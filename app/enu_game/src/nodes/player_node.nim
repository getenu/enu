...
import
  gdext/classes/[
    ...
    gdinputevent, gdinputeventmousemotion, gdinputeventscreentouch, gdinputeventscreendrag,
    gdinputeventjoypadbutton, gdinputeventjoypadmotion, gdinputeventpangesture,
    gdpackedscene, gdresourceloader, gdviewport, gdkinematiccollision3d
  ]
...
proc handle_collisions(self: PlayerNode, collisions: seq[gdref KinematicCollision3D]) =
...
method ready*(self: PlayerNode) {.gdsync.} =
  ...
  self.world_ray = if not state.nodes.game.is_nil: state.nodes.game.get_node("WorldRay").as(RayCast3D) else: nil
  ...
  gamepad_sensitivity = vector2(state.config.gamepad_sensitivity, state.config.gamepad_sensitivity)
  ...
  # Set up model watchers if model exists
  self.model.transform_value.changes:
    if added:
      self.global_transform = change.item

  # Set initial rotation from model
...
proc calculate_velocity(self: PlayerNode, move_direction: Vector3, delta: float): Vector3
...
method physics_process*(self: PlayerNode, delta: float) {.gdsync.} =
  ...
  let input_direction = if process_input: self.get_input_direction() else: vector3()

  # Calculate movement direction in world space (with Godot 3 style forward rotation)
  const forward_rotation = deg_to_rad(-90.0)
  let basis = self.camera_rig.global_transform.basis
  let right = vector3(basis.x) * input_direction.x
  let up = core.UP * input_direction.y
  let forward = vector3(basis.x).rotated(core.UP, forward_rotation) * input_direction.z
  ...
  # Calculate velocity with floating logic
  var velocity = self.calculate_velocity(move_direction, delta)

  # Update model input direction
  if not self.model.is_nil:
  ...
  # Handle collisions
  if process_input and not self.model.is_nil:
    ...
    # Handle auto-stepping for small obstacles
    if self.is_on_floor():
      self.boosted = false
    ...
    if move_direction.length() > 0.5:
      ...
      if self.down_ray.is_colliding():
        let length = 1.85
        let diff = length - (self.down_ray.global_position - self.down_ray.get_collision_point()).y
    ...
    # Safety: Reset position if fallen through world
...
method unhandled_input*(self: PlayerNode, event: InputEvent) {.gdsync.} =
    ...
    if index notin state.ignored_touches:
      if event.get_index() == 0:
        if event.is_pressed():
          ...
          if ?self.touch_position and self.touch_position.get() == event.get_position():
            self.update_raycast()
  ...
  elif event.is_class("InputEventScreenDrag") and TouchControls in state.local_flags:
  ...
  # Handle gamepad controller input for command mode
  if EditorVisible in state.local_flags and not self.skip_release and
     (event.is_class("InputEventJoypadButton") or event.is_class("InputEventJoypadMotion")):
  ...
  # Handle jump input with flying toggle
  ...
  # Handle crouch input with flying toggle
  ...
  # Handle run/alt speed input
  if event.is_action_pressed("run"):
    self.get_viewport().set_input_as_handled()
    let toggle = ?self.run_time and time < self.run_time.get() + alt_speed_toggle
  ...
  if event.is_class("InputEventPanGesture") and state.current_tool_value.value notin {CodeMode, PlaceBot}:
...
proc calculate_velocity(self: PlayerNode, move_direction: Vector3, delta: float): Vector3 =
  # Calculate speed based on flying state and alt speed modifiers
  let speed =
    if not self.flying() and not (self.alt_speed xor AltWalkSpeed in state.local_flags):
      float(state.config.walk_speed)
    elif not self.flying() and (self.alt_speed xor AltWalkSpeed in state.local_flags):
      float(state.config.alt_walk_speed)
    elif self.flying() and not (self.alt_speed xor AltFlySpeed in state.local_flags):
  ...
  if not self.flying():
    # Handle floating (extended jump time)
    let float_time = if self.alt_speed: float_time + float_time else: float_time
    let floating = self.jump_down and ?self.jump_time and
                   self.jump_time.get() + float_time > get_mono_time()

    let gravity = if floating: state.gravity / 4 else: state.gravity
    result.y = self.velocity.y + gravity * delta

proc get_input_direction(self: PlayerNode): Vector3 =
  if EditorVisible notin state.local_flags or CommandMode in state.local_flags or TouchControls in state.local_flags:
    result = vector3(
...
proc has_active_input(self: PlayerNode, device: int32): bool =
  ...
  for axis in 0 .. 9: # JOY_AXIS_MAX equivalent
    if axis != 6 and axis != 7 and # Left/Right triggers
        Input.get_joy_axis(device, JoyAxis(axis)).abs() >= 0.2:
      return true
  for button in 0 .. 22: # JOY_BUTTON_MAX equivalent
    if Input.is_joy_button_pressed(device, JoyButton(button)):
      return true

proc update_raycast*(self: PlayerNode) =
  let ray_length = if state.current_tool_value.value == CodeMode: 200.0 else: 100.0

  if MouseCaptured notin state.local_flags:
    let mouse_pos = self.get_viewport().get_mouse_position() * float(state.scale_factor)
    let cast_from = self.camera.project_ray_origin(mouse_pos)
    let cast_to = self.aim_ray.position + self.camera.project_ray_normal(mouse_pos) * ray_length

    if not self.world_ray.is_nil:
      self.world_ray.target_position = if ViewportFocused in state.local_flags: cast_to else: cast_from
      self.world_ray.position = cast_from

    # TODO: Implement aim_target.update() when AimTarget is available
...
proc init*(_: type PlayerNode): PlayerNode =
  let scene = cast[gdref PackedScene](ResourceLoader.load("res://components/Player.tscn"))
