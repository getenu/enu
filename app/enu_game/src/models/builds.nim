...
method on_begin_turn*(
  ...
  let map =
    if lean:
      {core.LEFT: core.BACK, core.RIGHT: core.FORWARD, core.BACK: core.RIGHT, core.FORWARD: core.LEFT}.to_table
    else:
      {core.LEFT: core.UP, core.RIGHT: core.DOWN, core.UP: core.RIGHT, core.DOWN: core.LEFT}.to_table
  let axis = map[axis]
  let move = self.is_moving(move_mode)
  if move:
